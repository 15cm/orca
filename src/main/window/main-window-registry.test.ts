...
describe('main-window-registry', () => {
  ...
  it('filters destroyed windows from broadcasts and lookups', () => {
    const first = createWindow(1)
    const second = createWindow(2)
  ...
  it('tracks live and visible windows', () => {
    const first = createWindow(1)
    const second = createWindow(2)
  ...
  it('does not touch destroyed window webContents', () => {
    ...
    expect(liveWindow.webContents.send).toHaveBeenCalledWith('pty:data', {
      id: 'pty-1',
      data: 'x'
    })
  })
  ...
  it('focuses existing window or opens a new one', () => {
    const first = createWindow(1)
