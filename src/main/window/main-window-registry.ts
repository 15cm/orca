...
export function getFocusedOrLastActiveMainWindow(): BrowserWindow | null {
  ...
  try {
    focusedWindow = BrowserWindow.getFocusedWindow?.() ?? null
  } catch {
    // Tests and non-window Electron surfaces may omit BrowserWindow.
  }
...
export function getMainWindowForWebContents(webContents: WebContents): BrowserWindow | null {
  ...
  try {
    window = BrowserWindow.fromWebContents?.(webContents) ?? null
  } catch {
    return null
  }
  if (!isLiveWindow(window)) {
...
export function getRegisteredMainWindow(
  ...
  let isBrowserWindow = false
  try {
    isBrowserWindow = window instanceof BrowserWindow
  } catch {
    return null
  }
  const browserWindow = window as BrowserWindow | null | undefined
  if (!isBrowserWindow || !isLiveWindow(browserWindow)) {
    return null
  }
  return windowsById.get(browserWindow.id) === browserWindow ? browserWindow : null
}
