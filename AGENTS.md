...
7. **Evidence-based blocker gate.** Every Sol review iteration must validate each finding with the relevant targeted unit/integration tests and, for UI or runtime behavior, an isolated `$gui-sandbox` scenario. Sol records whether the issue reproduces, the exact commands/scenario and results, and an automatic evidence classification: confirmed blocker when the required behavior fails, non-blocking when it does not reproduce and no contract violation is shown, or clarification required when evidence is inconclusive. Sol severity labels alone do not authorize fixes or tranche advancement.
8. **Human confirmation and iteration.** After every evidence-based Sol review, pause for explicit human review of each classification. The human may confirm or override it, recording rationale in the approved plan. Hand only human-confirmed blockers back to the same Luna execution thread. Luna fixes them and reruns affected validation, then Sol re-reviews the complete updated diff. A clean Sol review still requires explicit human approval before advancing. Repeat until the human approves closure and every approved validation step passes. Escalate conflicting feedback, changed requirements, or design changes to the human.
