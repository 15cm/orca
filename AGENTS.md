...
## Empty Project Windows

A window opened from a project group starts with nothing open, because the workspace session is one global object and the window that opened it already displays and writes it. Before touching `session:get` / `session:set` / `session:patch`, the startup hydration chain, or any code that writes the whole session rather than a patch, follow [`docs/reference/window-session-adoption.md`](./docs/reference/window-session-adoption.md): a window's adoption is frozen at creation and travels by argv, the launch's first window always hydrates the session, a window that opened empty must never write one back, and every new field of `WorkspaceSessionState` has to be classified carry/drop. That doc also holds the plan for partitioning the session per window.
